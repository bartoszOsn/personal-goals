...
export function BoardColumn<TData, TColumnId>(props: {
	...
	const itemsInColumn = props.boardProps.items
		.filter(item => props.groupedItemIds[`${props.column.columnId}`]?.includes(props.boardProps.itemIdSelector(item)));
	...
	return <Card w={props.boardProps.columnWidth}
		...
		<Stack w="100%">
			...
			{
				itemsInColumn.map((item, index) => <ItemCard key={props.boardProps.itemIdSelector(item)} item={item} index={index} column={props.column}

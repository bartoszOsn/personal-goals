...
import { InvalidWorkItemProgressTypeException } from '../../domain/work-item/error/InvalidWorkItemProgressTypeException';
...
export class WorkItemDomainErrorExceptionFilter extends DomainErrorExceptionFilter {
	protected override errorToHttp(
		...
		} else if (exception instanceof InvalidWorkItemProgressTypeException) {
			return new HttpError(BasicErrorDTOSchema, {
				code: HttpStatus.BAD_REQUEST,
				title: 'Invalid work item progress',
				message: exception.message,
				severity: 'error'
			});
